...
# from models.DeepSVDD.DetectorModel import DeepSVDD
# from models.HOC.model import base_Model
# from models.HOC.trainer import Trainer
from models.reasonable_metric import reasonable_accumulator
from ts_datasets.ts_datasets.anomaly import NAB, IOpsCompetition, SMAP, SMD, UCR, UCR_sub, SWAN, GECCO, ASD
...
parser.add_argument('--method', default='hoc', type=str,
                    help='Model: hoc, coca, svdd')
...
exec(f'from conf.{args.method}.{data_type}_Configs import Config as Configs')
...
if configs.dataset == 'UCR':
    configs.subdataset = selected_dataset 
...
if selected_dataset == 'NAB':
    ...
    dt = UCR()
elif selected_dataset in ['ECG', 'Gait', 'PowerDemand', 'EPG', 'ABP', 'RESP', 'Acceleration', 'NASA', 'AirTemperature']:
    dt = UCR_sub(family=selected_dataset)
elif selected_dataset == 'SMD':
    dt = SMD()
elif selected_dataset == 'SWAN':
    dt = SWAN()
elif selected_dataset == 'GECCO':
    dt = GECCO()
...
for idx in tqdm(range(len(dt))):
    ...
    # Load Model
    # 输入: configs, device, weight decay, train_dl, val_dl, test_dl, logger, idx
    ...
    test_score_origin, test_aff, test_score, score_reasonable, predict = Trainer(model, model_optimizer, train_dl,
                                                                                 ...
                                                                                 configs, idx)
    # 输出: test_score_origin, test_aff, test_score, score_reasonable, predict
